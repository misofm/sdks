...
import { getPartiesByIds, getPartyById } from "../src/queries.ts";
...
describe("getPartiesByIds", () => {
  ...
  it("rejects a Party from another partyos deployment", async () => {
    const staleId = id("3");
    const getObjects = mock(async () => ({
      objects: [
        {
    ...
    await expect(getPartiesByIds(client, [staleId], PKG)).rejects.toThrow(
      /a Party from a different partyos deployment/,
    );
  });
});
